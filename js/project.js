...
window.Specification = {
    ...
    highlightParts(partName) {
        ...
        const groupsToHighlight = this.structure.filter(item =>
            item.name.toLowerCase() === partName.toLowerCase()
        );

        if (groupsToHighlight.length > 0) {
            groupsToHighlight.forEach(group => {
                if (group.meshObjects && group.meshObjects.length > 0) {
                    group.meshObjects.forEach(mesh => {
                        if (!mesh.userData.originalMaterial) {
    ...
    renderSpecificationTable(structure) {
        ...
        structure.forEach((item, index) => {
            ...
            // Количество берем из модели
            const quantity = item.instanceCount || 1;

            // Добавляем CSS класс для строк без данных
        ...
        partRows.forEach(row => {
            row.addEventListener('click', () => {
                ...
                // Подсвечиваем деталь в 3D
                this.highlightParts(partName);
